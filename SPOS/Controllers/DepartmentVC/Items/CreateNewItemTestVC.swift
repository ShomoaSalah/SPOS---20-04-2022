...
class CreateNewItemTestVC: BaseVC {
...
//        removeNotificationObserver(.reloadCategories)
    ...
    @IBAction func submitAddItem(_ sender: Any) {
        ...
        guard categorySelectedIndex != 0 else {
            self.navigationController?.view.makeToast(NSLocalizedString("categorySelectedIndexRequired", comment: ""))
            return
        }
        guard !selectSoldBy.isEmptyStr else {
            self.navigationController?.view.makeToast(NSLocalizedString("selectSoldByRequired", comment: ""))
            return
        }
        guard let sku = skuTF.text, !sku.isEmptyStr else {
            self.navigationController?.view.makeToast(NSLocalizedString("skuRequired", comment: ""))
            return
        }
      
        guard let expireDate = expireDateTF.text, !expireDate.isEmptyStr else {
            self.navigationController?.view.makeToast(NSLocalizedString("expireDateRequired", comment: ""))
            return
        }
        ...
        AddNewItem(name: nameTF.text!, category_id: categorySelectedIndex, sold_by: selectSoldBy, price: priceTF.text ?? "", cost: amountTF.text ?? "", sku: skuTF.text!, bar_code: barcodeTF.text ?? "", date_expire: expireDateTF.text!, store_tracking: storeTracking, in_stock: inStockInt, color_id: colorSelectedID, image: itemImage.image ?? UIImage(), store_id: storeID, modifications_id: modificationsID, taxes_id: taxesID)
        
        
        print("itemImage.image \(itemImage.image)")
    }
...
extension CreateNewItemTestVC: UITableViewDataSource, UITableViewDelegate {
...
extension CreateNewItemTestVC: UICollectionViewDelegate, UICollectionViewDataSource {
...
extension CreateNewItemTestVC: UICollectionViewDelegateFlowLayout {
...
extension CreateNewItemTestVC {
    ...
    func getModificationsAndTaxes(store_id: Int)  {
        ...
        API.startRequest(url: requestUrl, method: .get, parameters: nil, viewCon: self) { [self] status, responseObject in
            ...
            if status {
                ...
                noDataView.isHidden = false
                print("ERROR 4 \(responseObject?.message ?? "")")
                self.view.makeToast(responseObject?.message ?? "")
            }
        }
    }
    ...
    func AddNewItem(name: String, category_id: Int, sold_by: String, price: String?, cost: String?, sku: String, bar_code: String?, date_expire: String, store_tracking: Int, in_stock: Int, color_id: Int?, image: UIImage, store_id: Int, modifications_id: [Int]?, taxes_id: [Int]?) {
        ...
        let urlRequest = APIConstant.addItem
        print("urlRequest add Item \(urlRequest)")
...
//        if itemImage.image != nil {
    ...
    //                self.navigationController?.view.makeToast(NSLocalizedString("updateProfileSuccessfully", comment: ""))

    //                postNotificationCenter(.reloadProfile, nil)
    //
...
extension CreateNewItemTestVC: ImagePickerDelegate {
